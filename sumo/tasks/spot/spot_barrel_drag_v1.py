...
@dataclass
class SpotBarrelDragConfig(SpotBaseConfig):
    """Config for the spot barrel dragging task."""
    ...
    w_goal: float = 500.0
    w_orientation: float = 50.0  # Reward for keeping barrel aligned with global x, y, z axes
    w_heading_alignment: float = 500.0  # Reward for barrel facing towards goal
...
class SpotBarrelDrag(SpotBase[SpotBarrelDragConfig]):
    """Task getting Spot to drag an upright crowd barrier to a goal location."""

    name = "spot_barrel_drag_v1"
    config_t = SpotBarrelDragConfig
    config: SpotBarrelDragConfig

    def __init__(self, config: SpotBarrelDragConfig | None = None) -> None:
